...
try {
    ...
    $stmt_cat = $conn->prepare("SELECT id FROM categories WHERE name = 'Domestic' LIMIT 1");
    $stmt_cat->execute();
    $categoryId = $stmt_cat->fetchColumn();
    if (!$categoryId) { throw new Exception("Domestic category not found."); }
...
<script>document.title = "Domestic Tours - Vogue Tourism";</script>

<main>
    <section class="page-banner hero-slider-section" style="background-image: url('<?php echo $baseURL; ?>/images/domastic-banner.webp'); height: 50vh;"></section>
    
    <div class="container">
        <section class="hero-search-bar">
            <!-- CHANGE 2: FORM ACTION URL -->
            <form action="<?php echo $baseURL; ?>/domestic/" method="GET" class="row g-3 align-items-center">
    ...
    <section class="section">
        <div class="container">
            <h2 class="section-title">All Domestic Tour Packages</h2>
            <?php if (isset($error)): ?><div class="alert alert-danger text-center"><?php echo htmlspecialchars($error); ?></div><?php endif; ?>
            
            <?php if (empty($tours)): ?>
                <div class="alert alert-info text-center mt-4">No domestic packages found matching your criteria.</div>
            <?php else: ?>
                <div class="row">
