...
try {
    ...
    // Get international category ID
    $stmt = $conn->prepare("SELECT id FROM categories WHERE name = 'International'");
    $stmt->execute();
    $internationalCategoryId = $stmt->fetchColumn();
    
    if (!$internationalCategoryId) {
        throw new Exception("International category not found");
    }
    
    // Get all international destinations
    $stmt = $conn->prepare("SELECT * FROM destinations WHERE category_id = ? ORDER BY name");
    $stmt->execute([$internationalCategoryId]);
    ...
    $toursQuery = "SELECT t.*, d.name as destination_name 
                  ...
                  WHERE d.category_id = ?";
    $params = [$internationalCategoryId];
...
<head>
    ...
    <title>International Tours - Vogue Tourism</title>
    <meta name="description" content="Explore the best international tour packages with Vogue Tourism. Find tours to popular destinations around the world.">
...
<body>
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav ms-auto">
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle active" href="#" role="button" data-bs-toggle="dropdown">International</a>
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">Domestic</a>
                             <ul class="dropdown-menu">
                                ...
                                <li><hr class="dropdown-divider"></li>
                                <?php 
                                // Get domestic destinations for dropdown
                                $stmt = $conn->prepare("SELECT d.* FROM destinations d JOIN categories c ON d.category_id = c.id WHERE c.name = 'Domestic' ORDER BY d.name LIMIT 10");
                                $stmt->execute();
                                $domDestinations = $stmt->fetchAll(PDO::FETCH_ASSOC);
                                
                                foreach ($domDestinations as $dest): 
                                ?>
                                <li><a class="dropdown-item" href="details.php?destination=<?php echo $dest['id']; ?>"><?php echo $dest['name']; ?></a></li>
                                <?php endforeach; ?>
                            </ul>
    ...
    <main>
            ...
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-lg-7">
                        ...
                        <p class="lead">Explore the world with our carefully crafted international tour packages.</p>
                    </div>
                    <div class="col-lg-5">
            ...
            <div class="filter-section">
                <form method="get" action="" class="row g-3">
                    <div class="col-md-8">
                        ...
                        <input type="text" class="form-control" id="duration" name="duration" placeholder="e.g. 5 Days" value="<?php echo $durationFilter; ?>">
                    </div>
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary me-2">Filter</button>
                        <a href="international-listing.php" class="btn btn-secondary">Reset</a>
                    </div>
                </form>
            </div>
        </section>
        ...
        <section class="container mb-5">
            <h2 class="text-center mb-4">Popular International Destinations</h2>
            <div class="row">
        ...
        <!-- Tours Section -->
        <section class="container mb-5">
            <h2 class="text-center mb-4">All International Tour Packages</h2>
            <?php if (isset($error)): ?>
